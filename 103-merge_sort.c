#include "sort.h"
#include <stdio.h>
#include <stdlib.h>
/**
 *_callooc - this is a calloc function
 *@nmembb: number of elemets
 *@size: bit size of each element
 *Return: pointer to memory assignement
 */
void *_callooc(unsigned int nmembb, unsigned int size)
{
	unsigned int i = 0;
	char *b;

	if (nmembb == 0 || size == 0)
		return ('\0');
	b = malloc(nmembb * size);
	if (b == '\0')
		return ('\0');
	for (i = 0; i < (nmembb * size); i++)
		b[i] = '\0';
	return (b);
}
/**
 *merge - make a merge
 *@array: one from start to mid
 *@a: temp array used in merge, was created outside to
 *optimize reducing the system calls
 *@start: first element position
 *@med: array middle
 *@end: last element position
 **/
void merge(int *array, int *a, int start, int med, int end)
{
	/*  sizes and temp arrays */
	int size_left = med - start + 1, size_right = end - med;
	int *array_left, *array_right;
	/* counters */
	int left, right, i = 0;

	array_left = &a[0];
	array_right = &a[size_right];
	for (left = 0; left < size_left; left++)
		array_left[left] = array[start + left];
	for (right = 0; right < size_right; right++)
		array_right[right] = array[med + 1 + right];
	left = 0, right = 0, i = start;
	/* merging tmp arrays into main array*/
	while (left < size_left && right < size_right)
	{
		if (array_left[left] <= array_right[right])
			array[i] = array_left[left], left++;
		else
			array[i] = array_right[right], right++;
		i++;
	}
	/* merging remaining left array into main array*/
	while (left < size_left)
		array[i] = array_left[left], left++, i++;
	/* merging remaining right array into main array*/
	while (right < size_right)
		array[i] = array_right[right], right++, i++;
	printf("Merging...\n");
	printf("[left]: ");
	print_array(array_left, left);
	printf("[right]: ");
	print_array(array_right, right);
	printf("[Done]: ");
	print_array(&arr[start], left + right);
}
/**
 *mergesort - function that sorts an array of integers
 *in ascending order using the Merge sort algorithm
 *@array: array of integers
 *@a: temp array used in merge, was created outside to
 *optimize reducing the system calls
 *@start: fisrt element position
 *@end: last element position
 *Return: void
 */
void mergesort(int *array, int *a, int start, int end)
{
	int med;

	med = (start + end) / 2;
	if ((start + end) % 2 == 0)
		med--;
	if (med >= start)
	{
		mergesort(array, a, start, med);
		mergesort(array, a, med + 1, end);
		merge(array, a, start, med, end);
	}
}
/**
 *merge_sort - function that sorts an array of integers
 *in ascending order using the Merge sort algorithm
 *@size: size of the list
 *@array: array of integers
 *Return: void
 */
void merge_sort(int *array, size_t size)
{
	int *a;

	if (!array || size < 2)
		return;
	a = _calloc(size, sizeof(int));
	mergesort(array, a, 0, size - 1);
	free(a);
}
